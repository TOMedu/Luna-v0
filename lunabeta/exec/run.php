<?php
/* Currently using v1 code, no machine learning code - will update repo soon */
/* Initialize Brain */
...
if (strpos($command,'tetris') !== false) {
    $return = "You are on, $s.";
    $extra = "<br><br><a onclick='loadtetris()' style='color:blue;'>Let's Play</a><script>var url='../modules/tetris';var title='Tetris | Luna';function loadtetris(){window.open(url, title, 'toolbar=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width=1000, height=900');}</script>";
    $m = 1;
...
mysql_query("INSERT INTO LunaRecords (id, command, ip, agent, date, time) VALUES('','$command2','$ip','$agent','$dateval','$timeval')") or die(mysql_error()); */
?>
...
<link href='http://fonts.googleapis.com/css?family=Roboto:100' rel='stylesheet' type='text/css'>
  ...
  <body style="font-family:'Roboto','Gill Sans MT','Calibri','Lucida Grande','Arial';font-size:48px;"><center>
    <body onLoad="speak('<?php if($multi==1){ echo $return[$r]; } elseif($say2!=""){ echo $say2; } else { echo $return; } ?>')"><?php if($multi==1){ echo $return[$r]; } else { echo $return; } ?> <?php echo $extra; ?>
    <div id="audio"></div>
  </body>
</html>